...
for (x in 1:dim(ctrl.rep.zt.scaled)[1]){
  if (ctrl.cor.removed[x, 1] < 0.8) {
    if (ctrl.cor.removed[x, 3] < 0.8){
      ctrl.rep1.zt.scaled[x, ] <- NA 
    }
    if (ctrl.cor.removed[x, 2] < 0.8){
      ctrl.rep2.zt.scaled[x, ] <- NA
    }}
  
  if (ctrl.cor.removed[x, 3] < 0.8) {
    if (ctrl.cor.removed[x, 2] < 0.8){
      ctrl.rep3.zt.scaled[x, ] <- NA
    }
  }
     }
...
for (x in 1:dim(rnase.rep.zt.scaled)[1]){
  if (rnase.cor.removed[x, 1] < 0.9) {
    if (rnase.cor.removed[x, 3] < 0.9){
      rnase.rep1.zt.scaled[x, ] <- NA 
    }
    if (rnase.cor.removed[x, 2] < 0.9){
      rnase.rep2.zt.scaled[x, ] <- NA
    }}
  
  if (rnase.cor.removed[x, 3] < 0.9) {
    if (rnase.cor.removed[x, 2] < 0.9){
      rnase.rep3.zt.scaled[x, ] <- NA
    }
  }
}

Nr <- c(1:dim(rnase.rep1.zt.scaled)[1])
rnase.with.proteinnumbers <- cbind(Nr, rnase.rep1.zt.scaled, Nr, rnase.rep2.zt.scaled, Nr, rnase.rep3.zt.scaled)

View( ctrl.cor.removed)
View( ctrl.rep2.zt.scaled)
View( ctrl.rep3.zt.scaled)
...
r1c.zt <- ctrl.rep1.zt.scaled
r2c.zt <- ctrl.rep2.zt.scaled
r3c.zt <- ctrl.rep3.zt.scaled

r1c0.zt <- ctrl.rep1.zt.scaled
r2c0.zt <- ctrl.rep2.zt.scaled
r3c0.zt <- ctrl.rep3.zt.scaled

r1c0.zt[is.na(r1c)] <- 0
r2c0.zt[is.na(r2c)] <- 0
r3c0.zt[is.na(r3c)] <- 0

rc0.zt = r1c0.zt + r2c0.zt + r3c0.zt

#ctrl.zt.reduced <- rc0.zt/(3-((sum(apply(r1c.zt,1,sum)) +  as.numeric(is.na(r2c.zt[x,])) + as.numeric(is.na(r3c.zt[x,]))))/25))

(apply(is.na(r1c.zt), 1, sum)+apply(is.na(r2c.zt), 1, sum)+apply(is.na(r3c.zt), 1, sum))
```
