...
```{r}
selec.shoulderregion.zt <- lapply(shoulderregion.zt, function(x) 
  if (length(x) < 3) {return(numeric(0))} 
  else {return(x)})

View(selec.shoulderregion.zt)
```

### 7.3 protein amount under global peaks shift 

#### 7.3.1 protein amount under global peaks shift based on MVM
We can test for proteins with a significant higher protein amount under the global peaks in the RNase group.
```{r}
signifikante_reihen_global_mvm <- data.frame(Zustimmung_global = ifelse(globalpeak.value.rnase.mvm > 1.5 * globalpeak.value.ctrl.mvm | globalpeak.value.ctrl.mvm > 1.5 * globalpeak.value.rnase.mvm, 1, 0))

rownames(signifikante_reihen_global_mvm) <- rownames(rnase.mvm)

#View(signifikante_reihen_global_mvm)
#dim(signifikante_reihen_global_mvm)
...
```{r}
signifikante_reihen_global_zt <- data.frame(Zustimmung_global = ifelse(globalpeak.value.rnase.zt > 1.5 * globalpeak.value.ctrl.zt | globalpeak.value.ctrl.zt > 1.5 * globalpeak.value.rnase.zt, 1, 0))

rownames(signifikante_reihen_global_zt) <- rownames(rnase.zt)

#View(signifikante_reihen_global_zt)
#dim(signifikante_reihen_global_zt)
```




#### 7.3.3 protein amount under global peaks shift based on MMS

```{r}
signifikante_reihen_global_mms <- data.frame(Zustimmung_global = ifelse(globalpeak.value.rnase.mms > 1.5 * globalpeak.value.ctrl.mms | globalpeak.value.ctrl.mms > 1.5 * globalpeak.value.rnase.mms, 1, 0))

rownames(signifikante_reihen_global_mms) <- rownames(rnase.mms)

#View(signifikante_reihen_global_mms)
#dim(signifikante_reihen_global_mms)
```
With this method we can´t find any Rdeeps for the mms normalized data because all under all global peeks of the control and the RNase group the protein amount is 1. 


### 7.4 protein amount under local peaks shift 

#### 7.4.1 protein amount under local peaks shift based on MVM

First we have to sum up the protein amount under all local peaks - for the control group 
```{r}
# Neue Tabelle zur Speicherung der aufsummierten Werte erstellen
localpeaks.sumvalues.ctrl.mvm <- data.frame(Summed_Values = numeric(length(localpeaks.values.ctrl.mvm)))

# Aufsummieren der Werte für jede Reihe
for (i in seq_along(localpeaks.values.ctrl.mvm)) {
  summed_values <- sum(localpeaks.values.ctrl.mvm[[i]])
  localpeaks.sumvalues.ctrl.mvm[i, "Summed_Values"] <- summed_values
}

#View(localpeaks.sumvalues.ctrl.mvm)
```

First we have to sum up the protein amount under all local peaks - for the RNase group 
```{r}
localpeaks.sumvalues.rnase.mvm <- data.frame(Summed_Values = numeric(length(localpeaks.values.rnase.mvm)))

for (i in seq_along(localpeaks.values.rnase.mvm)) {
  summed_values <- sum(localpeaks.values.rnase.mvm[[i]])
  localpeaks.sumvalues.rnase.mvm[i, "Summed_Values"] <- summed_values
}
...
selbst festgelegter Grenzwert 

```{r}
signifikante_reihen_local_mvm <- data.frame(Zustimmung_local = rep(0, nrow(localpeaks.sumvalues.rnase.mvm)))

for (i in 1:nrow(localpeaks.sumvalues.rnase.mvm)) {
  if (abs(localpeaks.sumvalues.rnase.mvm$Summed_Values[i] - localpeaks.sumvalues.ctrl.mvm$Summed_Values[i]) > 7.5)
  {
    signifikante_reihen_local_mvm$Zustimmung_local[i] <- 1
  }
}

rownames(signifikante_reihen_local_mvm) <- rownames(rnase.mvm)

#View(signifikante_reihen_local_mvm)
#dim(signifikante_reihen_local_mvm)
```
...
```
```{r}
View(localpeaks.values.rnase.mms)
```

### 7.5 dataframe with all parameters

```{r}
parameters_mvm <- cbind(signifikante_reihen_global_mvm, signifikante_reihen_local_mvm, shift_mvm_named)

parameters_zt <- cbind(signifikante_reihen_global_zt, signifikante_reihen_local_zt, shift_zt_named)

parameters_mms <- cbind(signifikante_reihen_global_mms, signifikante_reihen_local_mms, shift_mms_named)
```
...
lshift.mvm <- sample(1:100, length(precipitated.mvm), replace = TRUE)
