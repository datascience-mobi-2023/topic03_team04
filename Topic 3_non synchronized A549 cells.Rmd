...
### 7.3 protein amount under local peaks shift 

#### 7.3.1 protein amount under local peaks shift based on MVM

```{r}
View(localpeaks.values.ctrl.mvm)
...
```{r}
localpeaks.sumvalues.ctrl.mvm <- apply(ctrl.mvm, 1, function(x) max(x))
localpeaks.sumvalues.rnase.mvm <- apply(rnase.mvm, 1, function(x) max(x))

signifikante_reihen <- which(localpeaks.sumvalues.rnase.mvm > 1.2* localpeaks.sumvalues.ctrl.mvm)

#View(signifikante_reihen)
#length(signifikante_reihen)
...
```{r}
View(localpeaks.sumvalues.rnase.mvm)
dim(localpeaks.sumvalues.ctrl.mvm)
```



Option 2: Testen der Signifikanz mit T-Test (funktioniert noch nicht perfekt)
```{r}
localpeaks.sumvalues.ctrl.mvm <- apply(ctrl.mvm, 1, function(x) max(x))
localpeaks.sumvalues.rnase.mvm <- apply(rnase.mvm, 1, function(x) max(x))
...
signifikante_reihen <- which(localpeaks.sumvalues.rnase.mvm > localpeaks.sumvalues.ctrl.mvm)

# Berechnung der p-Werte mit einem statistischen Test (z. B. t-Test)
p_values <- numeric(length(localpeaks.sumvalues.ctrl.mvm))
for (i in 1:length(localpeaks.sumvalues.ctrl.mvm)) {
  p_values[i] <- t.test(ctrl.mvm[i,], rnase.mvm[i,])$p.value
}
signifikante_reihen <- signifikante_reihen[p_values[signifikante_reihen] < signifikanzniveau]

#View(signifikante_reihen)
```

#### 7.3.2 protein amount under local peaks shift based on z-Transformation
```{r}
...
#### 7.3.3 protein amount under local peaks shift based on MMS
```{r}
