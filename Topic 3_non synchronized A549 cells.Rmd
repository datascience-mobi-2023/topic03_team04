...
```{r}
n = 2
shift_zt_global <- maxpeaks.ctrl.zt - maxpeaks.rnase.zt
shift_zt_named <- shift_zt_global
for (i in 1:nrow(rnase.zt)) {
  if (shift_zt_named [i] < (-n)){shift_zt_named [i] <- "right shift"}
  else if (shift_zt_named [i] > n){shift_zt_named [i] <- "left shift"}
...
```
```{r}
shift_zt_global_df <- as.data.frame(shift_zt_global)
View(shift_zt_global_df)
```

```{r}
n = 3
shift_zt_named <- shift_zt_global_df
for (i in 1:nrow(shift_zt_global_df)) {
  if (shift_zt_named [i,1] <= -n){shift_zt_named [i,1] <- "right shift"}
  else if (shift_zt_named [i,1] >= n){shift_zt_named [i,1] <- "left shift"}
  else (shift_zt_named [i,1] <- "no shift")
}
View(shift_zt_named)
length(shift_zt_named[shift_zt_named == "right shift"])
length(shift_zt_named[shift_zt_named == "no shift"])
