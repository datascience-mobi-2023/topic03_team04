...
for (i in 1:num_rows) {
  ...
  if (sum(!is.na(row_data_ctrl)) >= 2) {
    ...
    p <- ggplot(df, aes(x = x))+
      ...
      xlab("Fractions") +
      ylab("Protein amount") +
      labs(title = paste("zt Transformation -",protein.names[i])) +
      scale_color_manual(values = c("RNASE" = "tomato", "CTRL" = "skyblue2"))
...
Nur temporär das kemans funktioniert  
...
peak.rnase <- peak.mvm.rnase

```

#### 7.1.1. Parameters

We want kmeans to consider different parameters for this we put them in a matrix.
Here we create and normalize/scale the matrix:
...
First we will set the varables:
```{r}
