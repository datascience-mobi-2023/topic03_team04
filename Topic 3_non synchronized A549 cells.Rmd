...
for (i in 1:num_rows) {
  ...
  if (sum(!is.na(row_data_ctrl)) >= 2) {
    df <- data.frame(x = 1:length(row_data),yctrl= as.numeric(row_data_ctrl),yrnase= as.numeric(row_data_rnase))
...
# Daten in Listen von Vektoren umwandeln
peaks_list <- as.list(as.data.frame(t(peaks)))
heights_list <- as.list(as.data.frame(t(heights)))
sds_list <- as.list(as.data.frame(t(sds)))

# Daten in ein DataFrame umwandeln
df <- data.frame(peaks = I(peaks_list), heights = I(heights_list), sds = I(sds_list))
...
# Gauß-Kurven für alle Zeile erstellen
x <- seq(1, 25, length.out = 100)
gaussians <- apply(df, 1, function(row) {
  y <- sapply(seq_along(row$peaks), function(i) {
    gaussian(x, row$heights[i], row$peaks[i], row$sds[i])
  })
  rowSums(y)
})
...
# Anzahl Graphen pro Plot bestimmen
nrow <- 2
ncol <- 2
...
for (i in 1:ncol(gaussians)) {
  plot(x, gaussians[,i], type = "l", lty = 1,
       main = paste("Row", i))
}
