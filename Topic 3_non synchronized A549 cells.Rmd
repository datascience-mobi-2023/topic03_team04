...
```{r}
MS_Table_Ctrl_rep1_MMS <-MS_Table_reordered[,1:25]
MS_Table_Ctrl_rep2_MMS <-MS_Table_reordered[,26:50]
MS_Table_Ctrl_rep3_MMS <-MS_Table_reordered[,51:75]
MS_Table_RNase_rep1_MMS <-MS_Table_reordered[,76:100]
MS_Table_RNase_rep2_MMS <-MS_Table_reordered[,101:125]
MS_Table_RNase_rep3_MMS <-MS_Table_reordered[,126:150]
```

On them we apply Min-Max-Scaling:
```{r}

# MS_Table_Ctrl_rep1_MMS
MS_Table_Ctrl_rep1_MMS_normalized <- t(apply(MS_Table_Ctrl_rep1_MMS, 1, function(x) {
  normalized <- (x-min(x))/(max(x)-min(x))
}))

# MS_Table_Ctrl_rep2_MMS
MS_Table_Ctrl_rep2_MMS_normalized <- t(apply(MS_Table_Ctrl_rep2_MMS, 1, function(x) {
  normalized <- (x-min(x))/(max(x)-min(x))
}))

# MS_Table_Ctrl_rep3_MMS
MS_Table_Ctrl_rep3_MMS_normalized <- t(apply(MS_Table_Ctrl_rep3_MMS, 1, function(x) {
  normalized <- (x-min(x))/(max(x)-min(x))
}))

# MS_Table_RNase_rep1_MMS
MS_Table_RNase_rep1_MMS_normalized <- t(apply(MS_Table_RNase_rep1_MMS, 1, function(x) {
  normalized <- (x-min(x))/(max(x)-min(x))
}))

# MS_Table_RNase_rep2_MMS
MS_Table_RNase_rep2_MMS_normalized <- t(apply(MS_Table_RNase_rep2_MMS, 1, function(x) {
  normalized <- (x-min(x))/(max(x)-min(x))
}))

# MS_Table_RNase_rep3_MMS
MS_Table_RNase_rep3_MMS_normalized <- t(apply(MS_Table_RNase_rep3_MMS, 1, function(x) {
  normalized <- (x-min(x))/(max(x)-min(x))
}))
```
