...
## Identification of global Peaks
...
```{r}
maxpeaks.rnase.zt <- apply(rnase.zt, 1, function(x){which.max(x)})
maxpeaks.value.rnase.zt <- apply(rnase.zt, 1, function(x){max(x)})

maxpeaks.ctrl.zt <- apply(ctrl.zt, 1, function(x){which.max(x)})
maxpeaks.value.ctrl.zt <- apply(ctrl.zt, 1, function(x){max(x)})

View(maxpeaks.ctrl.zt)
```
...
maxpeaks.rnase.zt <- apply(rnase.zt, 1, function(x){which.max(x)})
maxpeaks.value.rnase.zt <- apply(rnase.zt, 1, function(x){max(x)})
# View(maxpeaks.rnase.zt)

maxpeaks.ctrl.zt <- apply(ctrl.zt, 1, function(x){which.max(x)})
maxpeaks.value.ctrl.zt <- apply(ctrl.zt, 1, function(x){max(x)})
# View(maxpeaks.ctrl.zt)

View(shift_zt_global)
```
...
```{r}
shift_zt_global[c(296, 669, 869, 2673, 2981)]
```

```{r}
n = 2
shift_zt_global <- maxpeaks.rnase.zt - maxpeaks.ctrl.zt
shift_zt_named <- shift_zt_global
for (i in 1:nrow(rnase.zt)) {
  if (i %in% c(296, 669, 869, 2673, 2981)) {shift_zt_named[i] <- "percipitated"}
...
```{r}
maxpeaks.ctrl.zt.df <- as.data.frame(maxpeaks.ctrl.zt)
maxpeaks.rnase.zt.df <- as.data.frame(maxpeaks.rnase.zt)
```
...
n <- 3
km.mms <- kmeans(shift_zt_global, centers = n, nstart = 1)
```
...
peak.ctrl <- maxpeaks.ctrl.zt
peak.rnase <- maxpeaks.rnase.zt
shifttype <- km.mms$cluster
...
peaks <- data.frame(peak.ctrl, peak.rnase, shifttype)

#für den barplot muss der auch der shifttype als Dataframe dargestellt werden
number.of.shifttypes <- data.frame(value = shifttype)

#der shifttype muss als factor dargestellt sein, vector geht nicht
peaks$shifttype <- factor(peaks$shifttype)
...
dot.plot <- ggplot(peaks, aes(peak.ctrl,peak.rnase)) + 
  geom_point(aes(colour = shifttype), shape = 19, alpha = 0.5, size = 2)+
...
bar.plot <- ggplot(number.of.shifttypes, aes(factor(value)))+
  geom_bar(fill = col
           , width = 0.5)+
  theme_minimal()+
