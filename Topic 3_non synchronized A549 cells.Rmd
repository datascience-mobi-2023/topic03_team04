...
plots_ctrl_rnase.zt[[460]]
plots_ctrl_rnase.zt[[663]]
plots_ctrl_rnase.zt[[1798]]
plots_ctrl_rnase.zt[[878]]
```
Parameter 2
```{r}
print(plots_ctrl_rnase.zt[[663]])
```
Parameter 3
```{r}
print(plots_ctrl_rnase.zt[[1798]])
```
Parameter 4
```{r}
print(plots_ctrl_rnase.zt[[878]])
```

#### 2.6.5. Boundaries and Precipitated proteins
The local peakfinder can't find local maxima at the boundaries (fraction 1 and fraction 25) because they have only one neighbor fraction. Following conditions have to be met to be defined as a local peak at the boundary. Firstly, it has to have a higher y-value than the contiguous two fractions. Secondly, the protein amount of those three fractions has to be bigger than 10.


Also precipitated proteins are not descriped by the parameters and have to be identified separately. 
...
The z-Transformation does not work with df that have NA-values. This means we cannot use the ctrl.clean and rnase.clean df. We have to use the old ctrl.rep or the MS.Table.naremoved[-non.reproducible,1:75] dfs. On these we first perform z-Transformation and then we use the same algorithms as before to reduce the dataset in regards to reproducibility.
...
Interessante Proteine: AAAT_HUMAN Bad Proteins: ADA10_HUMAN
...
The following plots visualize our results. (Only considerung the global shifts):
...
Here we cluster the shifts. Sadly the clusters do **not** correspond to the shifting categories (left shift, no shift, right shift)
...
Here we cluster the precipitation.
...
Here we cluster the shoulderregions.
...
#### 10.1.1 Regression analysis only using global shift

zuerst wandeln wir die Information left shift, right shift und no shift in numerische Daten um

```{r}
library(dplyr)

shift_mvm_named <- as.data.frame(shift_mvm_named) %>%
  mutate(Shift = recode(shift_mvm_named, "left shift" = 1, "right shift" = 2, "no shift" = 0, .default = 0))

```

anschließend berechnen wir die Korrelation der Proteinmengen in Control und RNase für jedes Protein
...
#print(correlation.mvm)

```

Nun können wir mit Hilfe der linearen Regression Vorhersagen treffen. Die ersten 2500 Proteine werden genutzt, um das Model zu trainieren. Auf die restlichen wird dann die Vorhersage angewendet

```{r}
# Trainingssatz (Zeilen 1 bis 2500)
train_mvm <- data.frame(Korrelation = correlation.mvm[1:2500, ], Shift = shift_mvm_named[1:2500, 2])

# Lineares Regressionsmodell
model_mvm <- lm(Shift ~ Korrelation, data = train_mvm)

# Testdaten (Zeilen 2501 bis 3074)
test_mvm <- data.frame(Korrelation = correlation.mvm[2501:3074, ])

# Vorhersagen machen
predictions_mvm <- predict(model_mvm, newdata = test_mvm)

# Vorhersagen auf diskrete Werte abrunden
rounded_predictions_mvm <- round(predictions_mvm)



print(summary(model_mvm))

#print(rounded_predictions_mvm)

```

Auch bei der Vorhersage gilt: 0=no shift, 1=left shift, 2=right shift

### 10.2 Regression analysis z-Transformation

#### 10.2.1 Regression analysis only using global shift
...
shift_zt_named <- as.data.frame(shift_zt_named) %>%
  mutate(Shift = recode(shift_zt_named, "left shift" = 1, "right shift" = 2, "no shift" = 0, .default = 0))
...
```{r}
correlation.zt <-as.data.frame( sapply(1:nrow(ctrl.zt), function(i) cor(ctrl.zt[i, ], rnase.zt[i, ])))
rownames(correlation.zt) <- rownames(rnase.zt)


#print(correlation.zt)
...
```{r}
train_zt <- data.frame(Korrelation_zt = correlation.zt[1:2500, ], Shift = shift_zt_named[1:2500, 2])

model_zt <- lm(Shift ~ Korrelation_zt, data = train_zt)

test_zt <- data.frame(Korrelation_zt = correlation.zt[2501:3074, ])

predictions_zt <- predict(model_zt, newdata = test_zt)
rounded_predictions_zt <- round(predictions_zt)

print(summary(model_zt))
#print(rounded_predictions_zt)
...
#### 10.3.1 Regression analysis only using global shift
...
shift_mms_named <- as.data.frame(shift_mms_named) %>%
  mutate(Shift = recode(shift_mms_named, "left shift" = 1, "right shift" = 2, "no shift" = 0, .default = 0))
...
```{r}
correlation.mms <-as.data.frame( sapply(1:nrow(ctrl.mvm), function(i) cor(ctrl.mvm[i, ], rnase.mvm[i, ])))
rownames(correlation.mms) <- rownames(rnase.mms)


#print(correlation.mms)
...
```{r}
train_mms <- data.frame(Korrelation = correlation.mms[1:2500, ], Shift = shift_mms_named[1:2500, 2])

model_mms <- lm(Shift ~ Korrelation, data = train_mms)

test_mms <- data.frame(Korrelation = correlation.mms[2501:3074, ])

predictions_mms <- predict(model_mms, newdata = test_mms)
rounded_predictions_mms <- round(predictions_mms)

print(summary(model_mms))
#print(rounded_predictions_mms)
