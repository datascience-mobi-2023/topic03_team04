...
### 4.1. Peaks for data set with mean value - normalization
...
```{r}
peakidentif.ctrl <- ctrl.mvm
peakidentif.rnase <- rnase.mvm

sd.byrow.ctrl <- apply(peakidentif.ctrl, 1, function(x) sd(x[x < 8]))
sd.mean.ctrl <- mean(sd.byrow.ctrl)
sd.byrow.rnase <- apply(peakidentif.rnase, 1, function(x) sd(x[x < 8]))
sd.mean.rnase <- mean(sd.byrow.rnase)
