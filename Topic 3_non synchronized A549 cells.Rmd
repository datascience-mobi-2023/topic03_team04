...
=======
##4.line graphs
In the following code the rnase.zt dataframe is used as an example. If rnase.zt ist exchanged by any of the other names of the data frames, line graphs for this data frame will be created. 
```{r}
library(ggplot2)
...
```
Man kann nun aus der Liste auf die einzelnen Plots zugreifen.
Zum Beispiel: plots_RNase.zt[[1]] für den ersten Plot, plots_RNase.zt[[2]] für den zweiten Plot, usw.
...
for (i in seq_along(shoulderregion)) {shoulderregion[[i]] <- shoulderregion[[i]][!shoulderregion[[i]] %in% neighbors.peaks[[i]]]}
...
for (i in 1:length(peaks)) {
  x <- seq(1, 25, length.out = 100)
  y <- gaussian(x, heights[i], peaks[i], sds[i])
  
  plot(x, y, type = "l", lty = 1, main = paste("Row", i))
}
