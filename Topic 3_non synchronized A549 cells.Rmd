...
### 6.1. Shifts with Mean Value Method
...
### 6.2. Shifts with z-Transformation
...
### 6.3. Shifts with Min-Max Scaling
...
```{r}
# mvm
saveRDS(shift_mvm_named, file = "Non_synchronized_A549_shift_mvm_named")

# mms
saveRDS(shift_zt_named, file = "Non_synchronized_A549_shift_zt_named")

# mms
saveRDS(shift_mms_named, file = "Non_synchronized_A549_shift_mms_named")
```


## Comparison with dataset RDeep2.0

```{r}
shift.mvm.named <- readRDS("Non_synchronized_A549_shift_mvm_named")
shift.zt.named <- readRDS("Non_synchronized_A549_shift_zt_named")
shift.mms.named <- readRDS("Non_synchronized_A549_shift_mms_named")
```
...
Finally, we compare our results with the Database of RNA-Binding Proteins and their Functions from RBP2Go.

```{r}
shift.zt.named.df <- as.data.frame(shift.zt.named)

names.rbps.zt <-  rownames(subset(shift.zt.named.df, shift_zt_named %in% c("right shift", "left shift")))

names.non.rbps.zt <-  rownames(subset(shift.zt.named.df, shift_zt_named %in% c("no shift")))

rownames.zt <- rownames(subset(shift.zt.named.df, shift_zt_named %in% c("no shift","right shift", "left shift")))
