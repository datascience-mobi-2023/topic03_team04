...
calculation of sd to later sort small fluctuations out. 
```{r}
...
new names for the identification of the peaks between fraction 2 and 24. For each fraction the maximum value.
```{r}
...
Calculation of all peaks: global and local
```{r}
...
Shoulder regions: include values with high protein amount, which are not included in the local or global maxima.

1. generell identification of regions with high values
```{r}
...
2. identification of regions around local and global peaks
```{r}
...
View(neighbors.peaks)
```


Identification of the boundaries (fraction 1 and fraction 25)
```{r}
...
View(fraction25.peak.rnase)
View(boundary25.rnase)
View(boundary1.rnase)
View(fraction1.peak.rnase)
View(rnase.with.proteinnumbers)
```
precipitated proteins:
```{r}
precipitated.rnase <- which(rnase.zt[, 25] > rnase.zt[, 24] & rnase.zt[, 25] > rnase.zt[, 23] & rnase.zt[, 25] + rnase.zt[, 24] + rnase.zt[, 23] == 100)
precipitated.ctrl <- which(ctrl.zt[, 25] > ctrl.zt[, 24] & ctrl.zt[, 25] > ctrl.zt[, 23] & ctrl.zt[, 25] + ctrl.zt[, 24] + ctrl.zt[, 23] == 100)
precipitated.zt <- unique(c(precipitated.rnase,precipitated.ctrl))
precipitated.zt
precipitated.rnase
precipitated.ctrl
rnase.zt[fraction25.peak.rnase,]
```
```{r}
...
#})
Ersatzdecreaseincrease <- apply(rnase.zt, 1, function(x) {
    indices <- which(sapply(1:(length(x)-2), function(i) {
      !all(diff(x[i:(i+2)]) > 0) & !all(diff(x[i:(i+2)]) < 0)
    }))
    if (length(indices) > 0) {
      return(indices)
    } else {
...
for (i in seq_along(largevalues_without_maxima)) {
    largevalues_without_maxima[[i]] <- largevalues_without_maxima[[i]][largevalues_without_maxima[[i]] %in% Ersatzdecreaseincrease[[i]]]
}
...
my_quantile <- function(x ,probs) {
    max_val <- max(x)
    q <- quantile(x[x <= max_val & x != 0] ,probs = probs)
   }
wendepunkte.zt <- apply(rnase.zt ,1 ,my_quantile ,probs = probs)
View(wendepunkte.zt)
```


```{r}
gaussian <- function(x, a, b) {1 * exp(-((x - a)^2)/(2 * b^2))}
gaussian(rnase.zt[1,],seq(1,25)
```

Gaussian fit mit sd basierend auf maxpeaks.value
```{r}
gaussian <- function(x, a, b) {100/(sqrt(2*pi)*b) * exp(-((x - a)^2) / (2 * b^2))}

peaks.global <- maxpeaks
HWHM <- 0.5*peaks.global
sds <- 0.5*((HWHM)/sqrt(2 * log(2)))
# sds <- 100/maxpeaks.value*(sqrt(2*pi))
# View(sds)
...
for (i in 1:56) {x <- seq(1, 25, length.out = 250) 
                 y2 <- gaussian(x, peaks.global[i], sds[i])
plot(x, y2, type = "l", lty = 1, main = paste("Row", i))}

#for (i in 1:length(peaks.global)) {
 # x <- seq(1, 25, length.out = 100)
# y2 <- gaussian(x, heights[i], peaks.global[i], sds[i])
  
  # plot(x, y2, type = "l", lty = 1, main = paste("Row", i))
# }
```
Gaussian fit mit sd basierend 1-sigma und 2-sigma Intervall.
...
```{r}
#gaussian fit in ein bisschen präziser formuliert, ohne extra Listen und dataframes 
...
peaks.global <- maxpeaks
HWHM <- 0.5*peaks.global
heights <- maxpeaks.value* (sds * sqrt(2 * pi))
sds <- 0.5*((HWHM)/sqrt(2 * log(2)))


# Anzahl der Plots pro Zeile und Spalte festlegen
nrow <- 2
ncol <- 2

par(mfrow = c(nrow, ncol))

# Gauß-Kurven zeichnen
for (i in 1:24) {
  x <- seq(1, 25, length.out = 100)
  y2 <- gaussian(x, heights[i], peaks.global[i], sds[i])
...
The follwing code uses the dataframes from the z-Transformation as an example. If .zt is exchanged by .mvm or .mms the lines for the other normalization methods can be plotted. 
...
for (i in 1:num_rows) {
  ...
  if (sum(!is.na(row_data_ctrl)) >= 2) {
    ...
    # Erstelle den Plot mit ggplot2 
    p <- ggplot(df, aes(x = x, y = yctrl, color = "red")) +
      geom_line() +
      geom_point() +
      xlab("Fractions") +
      ylab("Protein amount") 
...
for (i in 1:num_rows) {
  ...
  if (sum(!is.na(row_data_ctrl)) >= 2) {
    ...
    p <- ggplot(df, aes(x = x))+
      geom_line(aes(y=yrnase,color="green"))+
      geom_point(aes(y=yrnase,color="green"))+
      geom_line(aes(y=yctrl, color="red"))+
      geom_point(aes(y=yctrl, color="red"))+
      xlab("Fractions") +
      ylab("Protein amount") 
...
plots_Ctrl_RNase.zt[[1]]
```
Thanks to the created list it is now possible to look at each protein seperatly.
For example: plots_Ctrl_RNase.zt[[1]] for the first, plots_Ctrl_RNase.zt[[2]] for the second, and so on. 
```{r}
