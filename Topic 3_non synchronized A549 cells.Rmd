...
peaks <- data.frame(peak.ctrl, peak.rnase, shift_named[,1])
number.of.clusters <- data.frame(value = shift_named[,1])
peaks$cluster <- factor(shift_named[,1])


dot.plot <- ggplot(peaks, aes(peak.ctrl,peak.rnase)) + 
  geom_point(aes(colour = shift_named[,1]), 
             shape = 19, 
             alpha = 0.5, size = 1.5, 
...
plot_RBP[[1]] <- dot.plot + annotation_custom(grob = ggplotGrob(bar.plot), 
                    xmin = 15, 
                    xmax = 26.75, 
                    ymin = -1, 
                    ymax = 8)
