...
Proteine bei denen eine Korrelation gut aber die anderen zwei schlecht sind leiden unter einem abweichenden Replikat. Dieses muss entfernt werden (wenn cor 1/2 und 2/3 schlecht sind ist Replikat 3 schlecht)
Alle Werte dieser schlechten Replikate werden auf NA gesetzt. 
Beim späteren Zusammenfügen werden die NAs ignoriert und nicht durch 3 sonder durch (3 - (anzahl der NA-Replikate)) geteilt

Das ganze wird erstmal getestet:
```{r}

View(ctrl.cor.naremoved)

ctrl.rep <- MS.Table.naremoved
if (ctrl.cor.naremoved[4,2] < 0.9){
  ctrl.rep[4,1:25] <- NA
  }
rnase.rep2 <- t(apply(rnase.rep, 1, function(t) {
  rnase.rep[1,t] <- NA
}))

View(rnase.cor.naremoved)
x = 17
rnase.rep <- MS.Table.naremoved[,76:150]


for (x in 1:dim(rnase.cor.naremoved)[1]){
  if (rnase.cor.naremoved[x, 1] < 0.9) {
    if (rnase.cor.naremoved[x, 3] < 0.9){
      rnase.rep[x, 1:25] <- NA 
    }
    if (rnase.cor.naremoved[x, 2] < 0.9){
...
Nr <- c(1:dim(rnase.cor.naremoved)[1])
gesamt <- cbind(Nr, rnase.rep[,1:25], Nr, rnase.rep[,26:50], Nr, rnase.rep[,51:75])
View(gesamt)

```

```{r}
rnase.rep <- MS.Table.naremoved
for (x in 1:dim(rnase.cor.naremoved)[1]){
  if (rnase.cor.naremoved[x, 1] < 0.9) {
    if (rnase.cor.naremoved[x, 3] < 0.9){
      rnase.rep[x, 1:25] <- NA 
...
View(rownames_to_column(rnase.rep))
```
