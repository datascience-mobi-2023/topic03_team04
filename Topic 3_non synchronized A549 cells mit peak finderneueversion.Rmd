...
```{r}
rnase3_peaks <- data.frame(rnase3_scaled)
...
largevalues <-apply(rnase3_scaled, 1,function(x) { largevalues <- which(x > 9)
               
})
#derzeit funktioniert das nicht 
setdiff(largevalues, peaks)
localmaxima <- peaks
setdiff(localmaxima, peaks)
View(maxpeaks)
View(localmaxima)
...
#boundary <- apply(rnase3_scaled, 1, function(x) {which(x[1] > 1 |  x[ncol(rnase3_scaled)] > 1)
